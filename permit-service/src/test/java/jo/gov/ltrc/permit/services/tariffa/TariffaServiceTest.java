...
class TariffaServiceTest {
    ...
    private List<ReturnInflationResponse> inflation;
//
...
//    }
    ...
    @Test
    @DisplayName("/api/tariff/find/inflation - ReturnInflation Procedure")
    void getInflation() throws Exception {
        ReturnInflationResponse inflations[]  = objectMapper.readValue(
                mockMvc.perform(post("/api/tariff/find/inflation")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{}")).andReturn().getResponse().getContentAsString(),
                ReturnInflationResponse[].class);

        this.inflation = Arrays.asList(inflations);
        ...
        if (inflation.size() > 0){
            mockMvc.perform(post("/api/tariff/find/inflation")
                    ...
                    .content("{}"))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.[0].inflationreferanceid" , greaterThan(0)));
        }else {
            mockMvc.perform(post("/api/tariff/find/inflation")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{}"))
